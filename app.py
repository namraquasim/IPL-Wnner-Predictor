...
def main():
    ...
    if flask.request.method == 'POST':
        ...
        input_variables.toss_winner.replace(['Sunrisers Hyderabad', 'Mumbai Indians', 'Gujarat Lions',
                             ...
                             'Kochi Tuskers Kerala', 'Pune Warriors', 'Rising Pune Supergiants'],
                              np.arange(0, 14), inplace=True)
        input_variables.toss_decision.replace(['Batting', 'Bowling'], [0, 1], inplace=True)
        ...
        return flask.render_template('main.html', original_input={'City ':city, ' Home Team ':Home, ' Away Team ': Away, ' Toss Winner ':toss_winner, ' Toss Decision ':toss_decision,
                                     ' Venue ':venue},
                                    result=prediction[0],
                                    )
